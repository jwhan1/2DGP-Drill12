...
class Zombie:
    ...
    def compare_ball(self):
        if self.ball_count >= play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS#접근
        else:
            return BehaviorTree.FAIL#도주

    def move_to(self, r=0.2):
        self.state='Walk'
    ...
    def build_behavior_tree(self):
        a1 = Action('이동하기', self.move_to)
        a2 = Action('방황하기', self.set_random_location)
        a3 = Action('소년한테 접근', self.move_to_boy)
        a4 = Action('소년한테 도주', self.move_from_boy)

        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        c2 = Condition('소년의 공이 더 적은가?', self.compare_ball)
        #방황
        root = move_to_target_location = Sequence('목표 지점으로 이동하기', a2, a1)
        #추적
        root = chase_boy = Sequence('소년을 추적', c2, a3)
        #도주
        root = chase_or_avoid = Selector('추적 또는 도주', chase_boy, a4)

        root = close_to_boy = Sequence('소년과 근접', c1, chase_or_avoid)
        
        root = chase_or_flee = Selector('추적/도주 또는 순찰', close_to_boy, move_to_target_location)
